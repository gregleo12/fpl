export interface ColumnDef {
  ...
  getClassName?: (value: any, player?: any) => string;
}
...
export const COMPACT_COLUMNS: ColumnDef[] = [
  ...
  },
  {
    key: 'elite_ownership',
    label: 'Elite',
    tooltip: 'Top 10K Ownership %',
    width: 70,
    format: (v) => v !== undefined && v !== null ? `${v.toFixed(1)}%` : '-',
    align: 'center'
  },
  {
    key: 'elite_delta',
    label: 'Δ',
    tooltip: 'Elite - Overall Ownership',
    width: 70,
    format: (v) => {
    ...
    getClassName: (v) => {
      if (v === undefined || v === null) return '';
      if (v >= 20) return 'deltaStrongPositive';
      if (v >= 5) return 'deltaPositive';
      if (v > -5) return 'deltaNeutral';
      if (v > -20) return 'deltaNegative';
      return 'deltaStrongNegative';
    },
    align: 'center'
  },
  {
