...
import { PlayerCell } from './OwnershipPlayerCell';
import { ColumnDef } from './OwnershipColumns';
  ...
  return (
    <tr className={`${styles.row} ${styles.clickableRow}`} onClick={onClick}>
      ...
      {columns.map((col) => {
        ...
        const customClass = col.getClassName ? col.getClassName(value, player) : '';

        return (
          <td
            key={col.key}
            className={`${styles.statsCell} ${customClass ? styles[customClass] : ''}`}
