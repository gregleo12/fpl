...
import { PlayersTable } from './OwnershipPlayersTable';
import { FilterModal, FilterState } from './OwnershipFilterModal';
...
export default function OwnershipPlayers() {
  ...
  async function fetchPlayers() {
    try {
      ...
      // Fetch all players with elite ownership (no pagination, sorted by total_points desc)
      const response = await fetch('/api/players?limit=1000&sort=total_points&order=desc&includeElite=true', {
        cache: 'no-store'
      });
